...
if (window.innerWidth < 1183) {
  ...
  function createBubble() {
    ...
    for (let i = 0; i < amount; i++) {
      ...
      let moveTime = setInterval(moveBubblee, speed);
      let bottomPosition = bubble.style.bottom.replace(/px/, '') * 1;
      let start = bottomPosition;
      function moveBubblee() {
        if (bottomPosition > board.offsetHeight + 100) {
          bubble.style.left = `${getRandomNumber(0, width - size)}px`;
